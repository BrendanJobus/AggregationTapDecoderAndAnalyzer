...
int main(int argc, char *argv[]) {
        ...
        while(pcap_next_ex(handler, &header, &packet) >= 0) {
                ...
                std::cout << std::hex << (bits = ntohs(ethernet->ether_type)) << '\n';
                ...
                std::cout << "Version: " << (bits = ntohs(aristaTypes->version)) << '\n';
                std::cout << "subType: " << (bits = ntohs(aristaTypes->subType)) << '\n';
                ...
                if (bits == 0x1) {
                        ...
                        std::cout << std::dec << "seconds: " << (bits = ntohl(aristaTime64->seconds)) << '\n';
                        std::cout << std::dec << "nanoseconds: " << (bits = ntohl(aristaTime64->nanoseconds)) << '\n';
                } else {
                        aristaTime48 = (struct sniff_arista_times_48*)(packet + SIZE_ETHERNET + ARISTA_TYPES_LENGTH);
                        std::cout << std::dec << "seconds: " << (bits = ntohl(aristaTime48->seconds)) << '\n';
                        std::cout << std::dec << "nanoseconds: " << (bits = ntohl(aristaTime48->nanoseconds)) << '\n';
                }
